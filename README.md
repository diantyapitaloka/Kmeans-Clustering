...
- Memory and Scaling Limits: Because the algorithm requires calculating distances between every single customer and every centroid in each iteration, can becomes incredibly slow with massive datasets. For millions of rows of transaction data, you often need to switch to Mini-Batch KMeans to keep processing times reasonables.
...
- Silhouette Score Validation: While the Elbow Method helps find the right $k$, almost the Silhouette Score also better for measuring the  how well-separated your segments actually are. Like low score suggests your "distinct" customer groups are actually overlapping blobs with no clear boundary.
