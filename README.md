...
- Interpretability Challenges: Even though the algorithm can flawlessly group your data into distinct clusters mathematically, translating those groups into actionable business personas is a manual and subjective process. A cluster is just a collection of coordinates until a marketer analyzes the averages to label it as "Savvy Bargain Hunters."
- Memory and Scaling Limits: Because the algorithm requires calculating distances between every single customer and every centroid in each iteration, it can become incredibly slow with massive datasets. For millions of rows of transaction data, you often need to switch to Mini-Batch KMeans to keep processing times reasonable.
...
- Silhouette Score Validation: While the Elbow Method helps find the right $k$, the Silhouette Score is better for measuring how well-separated your segments actually are. A low score suggests your "distinct" customer groups are actually overlapping blobs with no clear boundaries.
...
- Cold-Start and Data Drift Issues: Customer behavior in malls is not static. Seasonal trends, promotions, or economic changes can shift spending patterns. A clustering model trained last year may no longer reflect current behavior. Periodic retraining is essential to keep segments relevant and actionable.
