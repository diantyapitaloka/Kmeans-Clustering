...
- Local Optima Risk: The algorithm's finals result depends heavily on where the initial "seeds" or centroids are randomly placed. To avoid inconsistent results across different runs, always use the K-Means++ initialization to ensure a more stable and reproducible segmentation.
- Interpretability Challenges: Even though the algorithm can flawlessly group of your data some into distinct clusters mathematically, translating those groups into a actionable business personas is a manual and subjective process. A cluster is just a collection of coordinates until a marketer analyzes the averages to label it as "Savvy Bargain Hunters."
