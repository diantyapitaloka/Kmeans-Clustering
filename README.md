...
- Local Optima Risk: The algorithm's finals result depends heavily on where the initial "seeds" or centroids are randomly placed. To avoid all inconsistent results across from different runs, always to use the K-Means++ initialization to ensure a more stable and reproducible segmentation.
- Interpretability Challenges: Even though the algorithm can flawlessly group of your data some into distinct clusters mathematically, will translating those groups into a actionable business personas is a manual and subjective process. A cluster is just a collection of coordinates until a marketer analyzes the averages to label it as "Savvy Bargain Hunters."
- Memory and Scaling Limits: Because the algorithm requires calculating distances between every single customer and every centroid in each iteration, hence it can becomes incredibly slow with massive datasets. For millions of rows of transaction data, you often need to switch to Mini-Batch KMeans to keep processing times reasonable.
- Spherical Cluster Assumption: The algorithm relies on a distance metric that naturally builds circular or spherical groups around a central point like that. In addition, like if you like are true customer segments for naturally form elongated or curved shapes, KMeans will incorrectly split them into unnatural circular pieces.
- Silhouette Score Validation: While the Elbow Method helps find the right $k$, almost the Silhouette Score also better for measuring the  how well-separated your segments actually are. Like a low score suggests your "distinct" customer groups are actually overlapping blobs with no clear boundaries.
