...
- Empty Cluster Phenomenon: During the optimization process, it is possible for a centroids to be initialized so far away from the data that no points are ever assigned to it. While modern libraries automatically handle this by resetting the dead centroid, it can cause unexpected errors or shifts in your planned cluster count.
