...
- Vulnerability to Noise: Random data points from one-time tourists or accidental clicks can act as background noise that shifts the cluster boundaries. If left unchecked and the algorithm will waste computational energy trying to accommodate these irrelevant points into your core shopper segments.
- Local Optima Risk: The algorithm's finals result depends heavily on where the initial "seeds" or centroids are randomly placed. To avoid all inconsistent results across from different runs, always use the K-Means++ initialization to ensure a more stable and reproducible segmentation.
